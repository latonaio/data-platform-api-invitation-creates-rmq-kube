...
import (
	...
	"data-platform-api-invitation-creates-rmq-kube/config"
...
func (c *DPFMAPICaller) subfuncProcess(
	...
	for _, fn := range accepter {
		...
		case "ValidationCheck":
			c.validationCheckCreate(mtx, wg, subFuncFin, input, output, subfuncSDC, errs, log)
		default:
			wg.Done()
		}
	}
...
func (c *DPFMAPICaller) validationCheckCreate(
	mtx *sync.Mutex,
	wg *sync.WaitGroup,
	errFin chan error,
	input *dpfm_api_input_reader.SDC,
	...
	//}()
	//defer wg.Done()
	//err = c.complementer.ComplementValidationCheck(input, subfuncSDC, log)
	//if err != nil {
	//	mtx.Lock()
	//	*errs = append(*errs, err)
	//	mtx.Unlock()
